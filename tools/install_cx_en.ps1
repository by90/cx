# This file installs the English cx skills and refreshes the global Codex AGENTS.md file.
...
# The language package is fixed to English for this installer.
$languageSubpath = "en"
...
$sourceAgentsPath = Join-Path -Path $temporaryRoot -ChildPath "packages\en\AGENTS.md"
# The cloned English skill directory is the authoritative installation source.
...
try {
    ...
    # Enumerate every English cx skill from the cloned main branch.
    $sourceSkills = Get-ChildItem -LiteralPath $sourceSkillsDirectory -Directory | Sort-Object -Property Name
    # Remove every current English cx skill registration and directory before reinstalling.
    ...
    # Install every English cx skill from the repository default main branch.
    ...
    foreach ($sourceSkill in $sourceSkills) {
        ...
        if (-not (Test-Path -LiteralPath $installedSkillDirectory -PathType Container)) {
            ...
            throw "Installed English cx skill directory is missing: $($sourceSkill.Name)"
        }
        ...
        if ($signatureDifference) {
            ...
            throw "Installed English cx skill content does not match main: $($sourceSkill.Name)"
        }
    }

    # Replace the global Codex AGENTS.md with the English package template.
    ...
    if ($sourceHash -ne $installedHash) {
        ...
        throw "Global AGENTS.md hash does not match the English cx package template."
    }

    # Tell the user that every current skill was reinstalled and verified.
    Write-Host "Reinstalled and verified all English cx skills under $skillsDirectory"
