...
- 默认选择能满足当前需求的最少代码实现，不提前框架化、泛化或抽象；先证明真实调用点，再决定是否抽取公共入口。
...
5. 这个通用包是否有包内 `readme.md` 说明公共接口和使用方式？
...
- 包内 `readme.md`，列出公共接口和使用方式；不要把实例配置节、内部字段或实现步骤逐项写成接口文档。
...
- 交付物完成后的 `$cx-review` review 结论，重点说明是否消除了重复味道、是否仍保持极简公共入口、是否没有过度抽象或多余参数传递；交付前再记录 `$cx-evidence` 证据审查结论。
