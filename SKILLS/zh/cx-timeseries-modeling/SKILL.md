...
10. 需要用十分之一完整实体样本冻结用户明确固定的维度，搜索其余字段、标签、滑动窗口、模型结构、学习率范围、优化器和调度器，并按较小验证损失、较多验证损失有效改进次数、尽量跑满 120 个训练轮次和靠后的最佳轮次联合筛选 5 个候选时，叠加 `$cx-pytorch-quick-hpo`；需要保持候选的数据与模型参数不变，在全部样本上只调整批次大小、学习率、优化器和调度器参数，并逐一训练、测试和回测 5 个候选时，叠加 `$cx-pytorch-full-hpo`。
