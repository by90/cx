...
15. 训练早停只能监控验证损失的改进；早停计数按验证损失越小越好及冻结的最小改进量累计或重置。业务指标只用于逐轮观察、候选比较和独立业务最优检查点，不得参与早停。人工智能代理必须持续读取文件型训练轨迹，并在每个完整训练轮结束后立即报告轮次、耗时秒数、早停计数、验证损失和训练损失。股票排名训练还要分别按动态 TopN、固定 Top1、Top3、Top5、Top10 的顺序报告最高三级命中率与最低三级混入率。
...
- 参数试验的实际生效配置快照、产品默认配置未受影响的证据，以及人工智能代理持续读取的逐轮损失、验证损失早停、耗时、最高三级命中率和最低三级混入率报告；业务指标未参与早停的轨迹证据。
