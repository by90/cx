...
10. 需要用十分之一完整实体样本冻结用户明确固定的维度，搜索其余字段、标签、滑动窗口、模型结构、学习率范围、优化器和调度器，并按较小验证损失、较多验证损失有效改进次数、尽量跑满 120 个训练轮次和靠后的最佳轮次联合筛选 5 个候选时，叠加 `$cx-pytorch-quick-hpo`；需要在全部样本上每次只运行一个候选，结束后立即诊断收敛、速度、召回、业务风险和数据正确性，再依据证据决定下一次唯一调整时，叠加 `$cx-pytorch-full-hpo`。
...
- 与 `$cx-pytorch-quick-hpo` 共享的搜索空间、5 个轻量候选、逐候选消融与回测，以及 `$cx-pytorch-full-hpo` 每次一个全量候选的训练、测试、回测、强制诊断和证据驱动复跑结果。
