...
description: Use to discover and design registered common packages, shared utilities, reusable features, reusable classes, stable interfaces, and common data structures. Before implementation, read caller tutorials linked by project AGENTS.md, then search source and real callers so existing common capabilities are not ignored and duplicated.
version: 0.1.0
---
...
Find project capabilities before adding implementation. Every stable common capability stays small, has an independent caller tutorial under `docs/cx/docs/`, and is navigated by project `AGENTS.md`.

## Required discovery order

1. Identify the current domain from project `AGENTS.md`, then read every linked common-package tutorial and `docs/cx/docs/00.index.md`.
2. Obtain the common package, public interface, source location, usage, and scope from the tutorial.
...
Missing documentation does not prove that a capability is absent. Search source anyway. When a stable capability lacks a tutorial or project navigation, use `$cx-doc` to add the caller tutorial, topic index, and project `AGENTS.md` entry first.
...
## Caller tutorials and current state

- Every common package and stable public interface has one numbered caller tutorial under `docs/cx/docs/`.
- The tutorial states goal, scenarios, prerequisites, public entry, inputs, outputs, a minimal runnable example, steps, expected results, common failures, constraints, and verification.
- Project `AGENTS.md` registers domain, tutorial link, public entry, and read-first condition.
- A common package without a caller tutorial or project navigation is incomplete.
- Rewrite the same tutorial when its public entry changes. Never preserve migration history.
- A package-local note, when unavoidable, only links to the tutorial and shows the shortest entry. It does not duplicate details.
...
- Tutorial navigation in project `AGENTS.md`.
- Verification.
