...
description: Use for lightweight HPO in PyTorch, Lightning, and time-series projects; use exactly one tenth of complete-entity samples to select data, model-structure, and training parameters, then run ablation and backtesting for each of 5 candidates before full-data tuning.
version: 0.1.0
---
...
Lightweight HPO carries most parameter selection on a smaller but complete sample. It selects data parameters such as field combinations, feature modes, labels, and windows; model-structure and capacity parameters; and training parameters such as batch size, learning rate, optimizer, and scheduler. It keeps 5 candidates and completes per-candidate ablation and backtesting before full-data tuning.
...
5. Before lightweight tuning, study every field in the field combination for every candidate model against the label: positive contribution, negative contribution, and noise suspicion. Identify important fields, secondary fields, duplicated fields, and likely redundant fields.
...
13. Lightweight training has a fixed cap of `120` epochs, monitors `val_loss`, and uses fixed early-stopping patience `9`.
14. Every trial must record whether it completed all `120` epochs, the `val_loss` improvement count and improvement ratio, the best epoch, the final epoch, and the absolute and proportional distance from the best epoch to the final epoch.
...
21. Lightweight HPO keeps exactly `5` final candidates. Once candidates are fixed, run ablation and backtesting for every candidate. For ablation, hold that candidate's sample, seed, and training contract fixed while removing selected data groups, model-structure modules, or key training components one at a time, then record the change and conclusion.
22. Candidate screening prioritizes training sustainability: stronger signals come from more `val_loss` improvements, a higher improvement ratio, naturally completing `120` epochs, and a best epoch closer to the final epoch. When sustainability is comparable, use best/final `val_loss`, `test_loss`, and backtest results to rank candidates.
...
10. Rerun each stage's best recipe with the same seed config value `3407`, then record the best trial, candidates, each model's per-model best recipe, failed trials, pruned trials, five-minute tracking summaries, data version, field recommendations, and convergence evidence in the target `docs/cx` task, change, or experiment summary.
11. After all search stages, fix exactly `5` candidates. Rank sustainability first, then use `val_loss`, `test_loss`, and business metrics to distinguish otherwise comparable plans; a single high backtest must not displace a more sustainable candidate.
12. Run ablation and backtesting for each of the `5` candidates. Every ablation uses the same one-tenth complete-entity sample, seed config value `3407`, `120`-epoch cap, and early-stopping patience `9`; save each candidate's baseline, ablation variants, and backtest result before handing all `5` candidates to full HPO.
13. When the user requests business-first reporting, show the five candidates with Top10 hit rate, missed-candidate mean gain, downside share, downside mean loss, and `val_loss`, while preserving the sustainability and `val_loss` diagnostic views.
...
- `120`-epoch cap / patience `9` execution, early-stop epoch, best/final `val_loss`, `val_loss` improvement count and ratio, best epoch, final epoch, and absolute and proportional best-to-final-epoch distance.
...
- Five-candidate comparison table with Top10 hit rate, hit-candidate mean gain, missed-candidate mean gain, Top10 downside share, downside mean loss, sustainability evidence, `val_loss`, `test_loss`, trained epochs, trial duration, and average epoch duration.
- All `5` candidates' reruns, per-candidate ablation matrices, per-candidate backtests, full-data-tuning admission reasons, and reasons excluded plans did not enter the five-candidate set.
