...
15. Training early stopping may monitor validation-loss improvement only. Its counter advances or resets from lower-is-better validation loss and the frozen minimum improvement. Business metrics are limited to per-epoch observation, candidate comparison, and a separate business-best checkpoint; they never participate in early stopping. The agent must continuously read the file-backed training trail and immediately report epoch position, elapsed seconds, early-stopping counter, validation loss, and training loss after every complete epoch. A stock-ranking run must also report highest-three hit rate and lowest-three contamination separately, each ordered as dynamic TopN and fixed Top1, Top3, Top5, and Top10.
...
- The trial's effective-configuration snapshot, evidence that the product default configuration remained unchanged, and the per-epoch loss, validation-loss early-stopping, elapsed-time, highest-three-hit, and lowest-three-contamination reports continuously read by the agent, including trail evidence that business metrics did not participate in early stopping.
