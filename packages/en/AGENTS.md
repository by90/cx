...
27. **Trial-configuration isolation and per-epoch monitoring**: experimental candidate values such as learning rate, weight decay, batch size, and scheduler parameters are not unit-test contracts. When only a candidate value changes, do not create or edit unit tests and do not pin the concrete candidate value in an assertion. A trial should copy a typed configuration and override it in memory, or it may use a temporary configuration file owned only by that trial; neither approach may modify the product default configuration used for formal-release inference. After every complete training epoch, the agent must continuously read the real file-backed trail and immediately report epoch position, elapsed seconds, early-stopping counter, validation loss, and training loss. A stock-ranking run must also report highest-three hit rate and lowest-three contamination separately, each ordered as dynamic TopN and fixed Top1, Top3, Top5, and Top10.
