...
16. **Cross-platform encoding**: PowerShell scripts use UTF-8 with BOM; other source, Markdown, JSON, TOML, YAML, and text files use UTF-8 without BOM. When viewing, searching, or printing UTF-8-without-BOM files that contain Chinese in Windows or PowerShell, first set UTF-8 input, output, and pipeline encoding explicitly, for example `[Console]::InputEncoding`, `[Console]::OutputEncoding`, `$OutputEncoding`, and `-Encoding UTF8` on text commands. Do not treat mojibake console output, inline-script string argument corruption, terminal font gaps, or command-default encoding behavior as evidence that files are damaged. Do not batch-edit code, commands, documents, tests, or configuration because of display or shell-passing encoding issues. For exact location, prefer `Get-Content -Encoding UTF8`, `Select-String -Encoding UTF8`, and small `apply_patch` hunks anchored on functions, classes, headings, keys, and code structure. Modify file encoding or explain encoding details only when the file is actually encoded incorrectly, a patch fails, or the user explicitly asks.
