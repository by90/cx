...
10. **Development errors must surface**: unless business rules explicitly require degradation, recovery, or user-visible guidance, programming implementations must not catch, swallow, silently skip, default-fallback, empty-result-fallback, fake successful retries, or wrap any exception that would cause product problems. The test is simple: if this bug would harm the product when shipped, it must stop execution during development and surface to tests or callers.
11. **No fallback-style prechecks on performance paths**: do not use broad prechecks, per-item validity checks inside large loops, training-loop checks, hot-path fallbacks, or silent batch filtering to hide data errors or slow performance. Handle data validity at entrypoints, data preparation, test fixtures, or separate diagnostic tasks, and never use those checks to replace real failures.
12. **No automatic BDD for non-programming work**: do not create BDD automatically for ordinary non-programming tasks; if it is unclear whether behavior discovery or acceptance scenarios are needed, ask the user one minimal clarification question first.
13. **Visible todo list**: when starting any multi-step task, create a todo list in the conversation first; update item status as work proceeds, and do not finish the turn until every item is completed, canceled, or explicitly blocked with the reason stated.
14. **Cross-platform encoding law**: PowerShell scripts `*.ps1`, `*.psm1`, and `*.psd1` must be saved as UTF-8 with BOM so Windows PowerShell 5.1 can parse non-ASCII comments and source text correctly. All non-PowerShell files, including Python, shell, Markdown, JSON, TOML, YAML, text, source, and config files, must be UTF-8 without BOM; do not convert every file to BOM. When Python output is garbled in Windows terminals, fix the project `env.ps1` or PowerShell profile by setting `[Console]::InputEncoding`, `[Console]::OutputEncoding`, `$OutputEncoding`, `PYTHONUTF8=1`, and `PYTHONIOENCODING=utf-8`, and confirm the terminal font supports Chinese glyphs; do not add BOMs to `.py`, `.sh`, `.json`, `.toml`, or other non-PowerShell files as a fallback.
