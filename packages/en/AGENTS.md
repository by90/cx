...
26. **Never opportunistically fix out-of-scope issues**: coding, testing, refactoring, review, documentation, and configuration changes may address only the current task jointly defined by the user's current request, current use-case path, original task document, and active unfinished change. When an issue is unrelated to the current task, never modify, delete, format, refactor, add tests, add documentation, or adjust configuration opportunistically, regardless of how obvious, simple, severe, or aligned with best practices the agent believes it is. Put it only on an authorization-needed list that states the file, location, symptom, risk, and recommendation, and request the user's explicit authorization for that specific item. Do not act before that authorization is given. The agent's inference, preference, review opinion, or desire to improve the code is not authorization and must not expand task scope.
27. **Trial-configuration isolation and per-epoch monitoring**: experimental candidate values such as learning rate, weight decay, batch size, and scheduler parameters are not unit-test contracts. When only a candidate value changes, do not create or edit unit tests and do not pin the concrete candidate value in an assertion. A trial should copy a typed configuration and override it in memory, or it may use a temporary configuration file owned only by that trial; neither approach may modify the product default configuration used for formal-release inference. Training early stopping may monitor validation-loss improvement only. Its counter may advance or reset only from lower-is-better validation loss and the frozen minimum improvement. Business metrics are limited to per-epoch observation, candidate comparison, and a separate business-best checkpoint; they never participate in early stopping. After every complete training epoch, the agent must continuously read the real file-backed trail and immediately report epoch position, elapsed seconds, early-stopping counter, validation loss, and training loss. A stock-ranking run must also report highest-three hit rate and lowest-three contamination separately, each ordered as dynamic TopN and fixed Top1, Top3, Top5, and Top10.
