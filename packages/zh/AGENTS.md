...
12. **Python 脚本使用 uv Python**：运行 Python 脚本、测试、构建或工具命令时，必须使用 `uv` 安装或同步的 Python；可使用 `uv run python ...`、项目 `.venv` 中的 Python，或项目明确规定的 `uv` 管理入口。系统自带 `python`、`python3` 只能用于确认环境，不能代替项目 Python。
13. **中文表达不夹英文**：使用中文写说明、计划、总结、文档或注释时，优先使用完整中文表达；只有代码标识符、命令、路径、接口名、库名、协议、标准名、外部专名，或翻译后会造成歧义的常见英文可以保留。
...
- 运行 Python 脚本、测试、构建或工具命令时，必须确认实际解释器来自 `uv` 管理的环境；已进入项目 `.venv` 后，可按项目约定直接使用 `python ...`。
...
- 在 Windows 原生 Codex App 中，默认使用 PowerShell 运行命令和本地设置脚本；只有代理切换到 WSL 或项目明确位于 WSL 环境时，才使用 WSL 命令环境。不要用 `ps` 代称 PowerShell；如果 PowerShell 不可用，先安装或修复 PowerShell，再继续。
