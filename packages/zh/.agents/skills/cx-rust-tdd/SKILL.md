...
14. 在当前任务文档中记录验证证据和 `$cx-review` 统一审查结论；未完成变更文件只维护当前工作清单，完成后删除。
...
- 除非用户在当前请求中明确要求具体校验或错误处理，否则禁止自行增加校验后构造错误，也禁止捕获、转换、包装、吞掉、静默跳过、默认值兜底、空结果兜底、假装重试成功或降级处理错误。
- 底层错误必须保持原始类型和信息，通过 `?` 或等价直接传播并中止当前操作；测试不得要求把底层错误改写成另一种错误。
...
- 不创建自定义错误类型；只有用户当前明确要求调用方区分某项错误时，才实现该具体边界。
- 默认值优先通过函数或构造参数表达，不把简单路径、文件名或一次性默认值提升为模块级常量。
- 只保留当前行为需要的公开 API；不要增加调试入口、内存校验入口、扫描入口或预防未来需求的接口。
- 只实现和验证当前最新接口；删除旧入口、别名、适配层、桥接层、兼容分支、旧行为及其测试，所有调用方同步迁移。
