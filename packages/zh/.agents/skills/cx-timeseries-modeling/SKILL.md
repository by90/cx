...
10. 需要调参时叠加 `$cx-pytorch-hpo`：复用项目公共调参模块，在固定数据边界上每次只运行一个候选，以验证业务指标为首要目标完成轨迹分析后，才允许一个证据支持的下一改动。股票任务使用注册制起点后的全部合格实体，不抽取十分之一实体，也不再切换所谓轻量与全量阶段。
...
- 与 `$cx-pytorch-hpo` 共享的固定数据边界、调参范围清单、冻结基线、每次一个候选的训练与验证轨迹、过程分析、唯一下一改动，以及最终冻结配方的严格测试和回测。
