...
6. 测试命令和结果是否记录？
7. 多功能组项目是否把 `docs/` 根目录限制为索引和说明，并把具体文档放入 `docs/<feature-group>/`？
8. 是否创建了孤立规划文档？
9. 可复用组件是否先搜索已有实现、相关 skills 和 Common Module Registry，再抽取或说明不抽取理由？
10. Python 测试是否默认使用 `unittest`，除非项目有例外？
11. Rust 代码变更是否运行了必要 cargo 命令？
12. 工作是否满足提示词契约：目标、上下文、约束、必须遵循的流程、验证方式和交付物？
13. 最终摘要是否报告已运行命令、结果、跳过检查及原因、残留风险？
