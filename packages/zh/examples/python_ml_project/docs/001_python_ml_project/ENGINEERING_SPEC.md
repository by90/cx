...
这是 `001_python_ml_project` 功能组的长期研发主文档。所有项目都按多个功能组组织，本文件位于 `docs/001_python_ml_project/ENGINEERING_SPEC.md`，并从 `docs/INDEX.md` 链接到这里。

## 0. Document Rules

- 本功能组的架构、任务队列、测试矩阵、可复用功能/类/组件决策和验证证据都放在这里。
- 需要行为发现时，BDD 场景放在同目录 `BDD.md`，其标题和 `Feature:` 名称必须与 `001_python_ml_project` 一致。
