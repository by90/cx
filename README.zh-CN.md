...
2. `$cx-story` 维护 `docs/cx` 下的用例、设计、任务和变更文档；所有需求都用“主成功场景 + 分支场景”表达。
...
8. 一个主成功场景文件夹只承载一个用户目标用例；主成功场景通常 3 到 9 步，从触发写到目标达成，不把多个互斥选择、页面按钮或完整任务堆进同一个场景。
9. 如果主成功场景某一步本身需要参与者、前置条件、步骤和分支，说明它已经是子用例或独立用例，应拆成新的主成功场景文件夹，并在 `docs/cx/00.项目说明.md` 中维护索引。
10. 开工前先询问执行模式：选择“直接做完文档、测试、实现和验证”，还是“每完成一个任务后征求同意”。
11. 如果用户没有明确选择逐任务确认，默认直接做完；完成用例、设计、任务拆分或变更记录后不停止，继续由 `$cx-tdd` 执行最窄失败测试、最小实现和重构。
12. 只有用户明确选择逐任务确认时，才在每个任务完成并记录验证证据后停止等待核对。
13. `$cx-pytorch-tdd`、`$cx-rust-tdd` 和 `$cx-common-module` 补充语言、框架和复用入口约束。
...
| `$cx-story` | 用例、主成功场景、分支场景、任务文件夹和变更文件夹 |
