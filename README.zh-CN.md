...
cx 是用于规范人类与人工智能协作研发的 Codex 技能包。它要求人工智能代理先读取 `docs/cx/docs/` 中的专题文档和项目已经登记的通用能力，再用 `docs/cx` 中的当前用例、设计、固定任务和临时变更文件推进工作；单元测试和测试先行开发只有明确声明时才进入任务范围。
...
2. `$cx-doc` 先读取 `docs/cx/docs/` 专题文档和已登记通用能力；`$cx-story` 再维护当前用例、设计和固定任务集合。
...
13. 任一交付物完成后必须由 `$cx-review` 连续执行交付物质量审查和完成证据门禁；通过后删除当前变更文件。
...
`docs/cx/docs/` 保存通用包和稳定技术过程的独立编号专题文档；`docs/cx/notes/` 保存针对具体研究问题的当前结论。所有正式文档只写当前状态，不保存新旧差异或完成变更历史。
