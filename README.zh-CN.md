...
代码写完后必须先做 review，再认定任务完成。review 不通过时，当前任务保持未完成，必须先修复文档不一致、重复味道、非 OOP、臃肿实现、多余校验、多余变量传递、多余命名或业务语义偏差。
...
13. 生产代码写完后必须由 `$cx-evidence` 执行本地代码 review；review 通过后才能把任务或变更标记完成。
...
| `$cx-evidence` | 代码完成后的强制 review、交付前证据审查、文档一致性、代码味道和剩余风险 |
