...
8. 开工前先询问执行模式：选择“直接做完文档、测试、实现和验证”，还是“每完成一个任务后征求同意”。
9. 如果用户没有明确选择逐任务确认，默认直接做完；完成用例、设计、任务拆分或变更记录后不停止，继续由 `$cx-tdd` 执行最窄失败测试、最小实现和重构。
10. 只有用户明确选择逐任务确认时，才在每个任务完成并记录验证证据后停止等待核对。
10. `$cx-pytorch-tdd`、`$cx-rust-tdd` 和 `$cx-common-module` 补充语言、框架和复用入口约束。
